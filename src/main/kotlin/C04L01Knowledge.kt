...
fun main() {
    ...
    val aiDevsClient = AiDevsClient(apiKey, 120000, "knowledge")
    val openAiClient = OpenAiClient(System.getenv("SPRING_AI_OPENAI_API_KEY"))

    C04L01Knowledge(aiDevsClient, openAiClient)
        .resolve()
}

class C04L01Knowledge(val aiDevsClient: AiDevsClient, val openAiClient: OpenAiClient) {

    private fun findExchangeRate(question: String): String {
        TODO("Not yet implemented: exchange rate")
    }

    private fun findCountryPopulation(countryName: String): String {
        TODO("Not yet implemented: population")
    }

    fun resolve() {
        val question = getQuestion(aiDevsClient)
        when (categorizeQuestion(question)) {
            "exchange rate" -> findExchangeRate(question)
            "country population" -> findCountryPopulation(question)
            "general knowledge" -> answerGeneralQuestion(question)
